...
typedef struct
{
  char   cmd_str[CLI_CMD_NAME_MAX]; // cmd
  void (*cmd_func)(cli_args_t *);   // callback - func pointer
} cli_cmd_t;
...
typedef struct
{
  ...
  uint16_t    cmd_count;  // 추가된 명령어 개수
  cli_cmd_t   cmd_list[CLI_CMD_LIST_MAX];
  cli_args_t  cmd_args;   // 명령어에서 사용할 인자들
} cli_t;
...
bool cliUpdate(cli_t *p_cli, uint8_t rx_data)
{
  ...
  switch(p_cli->state)
  {
    ...
    case CLI_RX_SP3:
      ...
      {
        if (line->cursor > 1) // home을 연속으로 눌렀을 때 한칸씩 앞으로 이동하는 것 방지
        {
      ...
      if (rx_data == CLI_KEY_END)
      {
        ...
        cliUpdate(&cli_node, CLI_KEY_ENTER); // EOF 문제 해결하기 위해 추가!!
      }
      break;
...
bool cliRunCmd(cli_t *p_cli)
{
  ...
  if (cliParseArgs(p_cli) == true) // 문자열이 입력되면 파싱 시작
  {
    cliPrintf("\r\n");
    ...
    { // 동일한 명령어가 있으면
      ...
      {
        p_cli->cmd_args.argc =  p_cli->argc - 1; // 명령어 자신은 제외
        p_cli->cmd_args.argv = &p_cli->argv[1];  // 명령어 다음의 인수들만 선택
        p_cli->cmd_list[i].cmd_func(&p_cli->cmd_args); // 명령어를 인자와 함께 실행
        break;
      }
    }
  }
...
bool cliAdd(const char *cmd_str, void (*p_func)(cli_args_t *))
{
  ...
  // 대소문자를 구분하지 않기 위해
  cliToUpper(p_cli->cmd_list[index].cmd_str); // 대문자로 변경
